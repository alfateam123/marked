...
var AJAX = function(CORS){
  ...
  var _request = function(req) {
      ...
      if (xhr.readyState == 4 && xhr.status == 200) {
        ...
        if(xhr.readyState == 4) { //call error function only when request has completed
          ...
          if(xhr.responseText.length > 0){
            ...
            clearTimeout(timeout);
            req.success(
              req.dataType ? ( req.dataType.toUpperCase() === "JSON" ? JSON.parse(xhr.responseText)
                  : (req.dataType.toUpperCase() === 'XML'? xhr.responseXML :  xhr.responseText) ) 
              : xhr.responseText
              );
            return;
          }
    ...
    if(req.type.toLowerCase() == 'get') {
      ...
      xhr.open("POST", req.url , /*true*/false);
      // Set header so the called script knows that it's an XMLHttpRequest
      xhr.setRequestHeader("X-Requested-With", "XMLHttpRequest");
